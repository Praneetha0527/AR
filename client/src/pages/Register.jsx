...
const Register = () => {
    ...
    const handleSubmit = (e) => {
        ...
        register(formData).then((res) => {
            ...
            alert("Registration successful! Welcome to Bharani Silks.");
            navigate("/");
        }).catch((err) => alert(err.response?.data?.message || "Error registering"));
    };
    ...
    return (
        <div className="min-h-[80vh] flex items-center justify-center px-4 bg-gray-50">
            <div className="max-w-md w-full bg-white p-10 rounded-2xl shadow-xl shadow-black/5 border border-gray-100">
                ...
                <h2 className="text-3xl font-serif text-center mb-8 font-bold text-[#2d2a26]">Create Account</h2>
                <form className="space-y-6" onSubmit={handleSubmit}>
                    <div>
                        <label className="text-sm font-semibold text-gray-400 block mb-2 uppercase tracking-widest text-[10px]">Full Name</label>
                        <input type="text" required onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full h-12 bg-gray-50 border border-gray-200 rounded px-4 focus:ring-1 focus:ring-red-700 focus:border-red-700 outline-none transition" />
                    </div>
                    <div>
                    ...
                    <button className="w-full h-12 bg-[#b12a32] text-white rounded font-bold shadow-lg active:scale-95 transition-transform uppercase tracking-widest text-xs">Create</button>
                </form>
                <div className="mt-8 text-center text-xs tracking-wide">
                    <span className="text-gray-400">Already have an account?</span> <Link to="/login" className="text-red-700 font-bold hover:underline ml-1">Login here</Link>
                </div>
            </div>
        </div>
    );
